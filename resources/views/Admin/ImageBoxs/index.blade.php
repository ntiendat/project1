...
@section('content')
    ...
    <div class="row" style="position: relative;">
        ...
        <div class="form div-select " style="">
            <div class="multiselect">
                ...
                <div id="checkboxes">
                  ...
                  <div><input type="checkbox" name="anhbaiviet" />Ảnh </div>
        ...
        <div class="form div-add" style="">
              <a href="{{route('create.image.box')}}" class="btn btn-warning" style="margin-bottom: 5px"><i class="fa fa-plus"></i> Thêm mới</a>  
        </div>
    </div>
    <br>
     <table id="someTable" class="table table-striped table-hover table-primary" >
            <thead >
                <tr>
                    ...
                    <th scope="col" class="anhbaiviet">Ảnh</th>
            ...
            <tbody id="tbody">
               <?php 
                    $stt = 1;
                ?>
               @foreach($image_boxs as $image_box)
               <tr>
                <th class="checkbox1"><input type="checkbox" class="checkbox"  name="" data-id="{{$image_box->id}}"></th>
                <td class="stt">{{ $stt++ }}</td>
                <td>{{ $image_box->title}}</td>
                <td>{!! $image_box->description !!}</td>
                <td>@if($image_box->media_id != 0)<img src="{{asset('Media/'.$image_box->media_url) }}" alt="" width="50px" height="50px"> @endif
                </td>
                <td>{{ $image_box->url}}</td>
                <th scope="col" class="tacvu">
                    <a href="{{route('edit.image.box',['id'=>$image_box->id])}}"><i class="fa fa-edit tacvu"></i></a>
                    <a class="delete" data-id={{$image_box->id}}><i class="fa fa-trash tacvu" style="color: red"></i></a>
                </th>
                </tr>
                @endforeach
...
@section('script-bottom')
 <script type="text/javascript">
            ...
            $('.delete').on('click',function(){
              ...
              if(confirm("Bạn có chắc muốn xoá Image box")){  
                    $.ajax({
                        url: '{{ route('delete.image.box') }}',
                        type: 'GET',
                        ...
                        success: function (data) {
                            ...
                            tr.remove();
                            showMessage("Image box đã được xoá",1);
                        },error: function (data) {
                            showMessage("Image box xoá bị lỗi",2);
                        }
                    });
                }
            });
            ...
            $('#actions').on('change',function() {
                if ($("#actions option:selected").val() == 1) {
                    ...
                    if (idsArr.length <=0)  {  
                        ...
                        if(confirm("Bạn có chắc muốn xoá các image box đã chọn")){  
                            var strIds = idsArr.join(","); 
                            $.ajax({
                                url: '{{route('delete-multiple.image.box')}}',
                                type: 'GET',
                                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                                data: {ids:strIds},
                                success: function (data) {
