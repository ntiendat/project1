...
@section('content')
    ...
    <div class="row" style="position: relative;">
        ...
        <div class="form div-select " style="">
            <div class="multiselect">
                ...
                <div id="checkboxes">
                  ...
                  <div><input type="checkbox" name="baiviet" />Sản phẩm</div>
        ...
        <div class="form div-add" style="">
            ...
            <select name="status-comment-product" class="form-control abc" id="status-comment-product">
                <option value="0">--</option>
                <option value="1">Chờ duyệt</option>
                <option value="2">Duyệt</option>
            </select>
             {{--  <a href="{{route('success.index.comment.product')}}" style="margin-bottom: 5px">Danh sách bình luận đã được duyệt</a>   --}}
        </div>
    </div>
    <br>
     <table id="someTable" class="table table-striped table-hover table-primary">
            <thead >
                <tr>
                    ...
                    <th scope="col" class="danhgia">Đánh giá</th>
                    <th scope="col" class="baiviet">Sản phẩm</th>
            ...
            <tbody>
               ...
               <tr>
                    ...
                    <td>{{ $comment->title}}</td>
                    <td>
                        <select  class="form-control optionstatus" data-id="{{$comment->id}}">
                          <option value="0">--</option>
        ...
        <div id="pagination" class="row">{{$comments->links()}}</div>
@endsection
...
<script type="text/javascript">
            ...
            function saveStatus(id,status) {
                $.ajax({
                    url: '{{route('update.status.comment')}}',
                    type: 'POST',
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    data: {id:id,status:status},
                    success: function (data) {
                       showMessage("Trạng thái đã được sửa",1);
                    },
                    error: function (data) {
                        alert(data);
                    }
            ...
            $("#status-comment-product").on('change',function(){
               var status = $("#status-comment-product option:selected" ).val();
               // alert(status);

                $.ajax({
                    url: '{{route('list.comment')}}',
                    type: 'GET',
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    data: {'status':status},
                    success: function (data) {
            ...
            $('#actions').on('change',function() {
                if ($("#actions option:selected").val() == 1) {
                    ...
                    if (idsArr.length <=0)  {  
                        ...
                        if(confirm("Bạn có chắc muốn xoá các bình luận đã chọn")){  
         ...
         // //tim kiem bang ten ,email ,dia chi,sdt
            $('#search').on('keyup',function(){
                $value = $(this).val();
                $status = $("#status-comment-product").val();
                //alert($value);
                $.ajax({
                    type: 'get',
                    url: '{{ route('searchComment.product') }}',
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    data: {
                        'search': $value,
                        'status': $status,
             ...
             function fetch_data_search(search_text,page)
              {
                //alert(search_text);
                  $.ajax({
                      type: "GET",
                      url: 'searchCommentproduct?page=' + page,
                      headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
