...
@section('content')
    ...
    <div  class="row" style="position: relative;">
        ...
        <div class="form div-add" style="">
              <a href="{{route('create.tag.product')}}" class="btn btn-warning" style="margin-bottom: 5px"><i class="fa fa-plus"></i> Thêm mới</a>  
        </div>
    </div>
    <br>
            ...
            <tbody>
               ...
               <?php $stt = 1; ?>
                   ...
                   <tr>
                    ...
                    <td class="stt">{{ $stt++ }}</td>
                    <td> {{substr($tag->name,0,50)}}</td>
                    <th scope="col" class="tacvu">
                        <a href="{{route('edit.tag.product',['id'=>$tag->id])}}"><i class="fa fa-edit tacvu"></i></a>
...
@section('script')
 ...
 <script type="text/javascript">
        ...
        $('.delete').on('click',function(){
              ...
              if(confirm("Bạn có chắc muốn xoá thẻ sản phẩm")){  
                    $.ajax({
                        url: '{{ route('delete.tag.product') }}',
                        type: 'GET',
                        ...
                        success: function (data) {
                            ...
                            tr.remove();
                            showMessage("Thẻ sản phẩm đã được xoá",1);
                        },error: function (data) {
                            showMessage("Thẻ sản phẩm xoá bị lỗi",2);
                        }
                    });
                }
            });
       ...
       $('#actions').on('change',function() {
        if ($("#actions option:selected").val() == 1) {
            ...
            if (idsArr.length <=0)  {  
                ...
                if(confirm("Bạn có chắc muốn xoá các thẻ tag đã chọn")){  
                    var strIds = idsArr.join(","); 
                    $.ajax({
                        url: '{{route('delete-multiple-tag-product.product')}}',
                        type: 'GET',
                        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                        data: 'ids='+strIds,
                        success: function (data) {
