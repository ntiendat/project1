...
@section('content')
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="#">Danh sách loại sản phẩm</a></li>
      </ol>
    </nav>
    <div id="page-wrapper">
        <div class="row" style="position: relative;">
            ...
            <div class="form div-select" style="">
                <input type="text" placeholder="Tìm kiếm..." id="searchCateProa">
            </div>
            ...
            <div class="form div-select " style="">
                <div class="multiselect">
                    ...
                    <div id="checkboxes">
                      ...
                      <div><input type="checkbox" name="anh" />Ảnh</div>
            ...
            <div class="form div-add" style="">
                  <a href="{{route('create.category.product')}}" class="btn btn-warning" style="margin-bottom: 5px"><i class="fa fa-plus"></i> Thêm mới</a>  
            </div>
        </div>
        <br>
         <table id="someTable" class="table table-striped table-hover table-primary" >
            <thead >
                <tr class="text-center">
                    <th class="checkbox1"><input type="checkbox"  name="" id="check_all" class="checkbox"></th>    
                    <th scope="col" class="stt">STT</th>
                    <th scope="col" class="ten">Tên</th>
                    <th scope="col" class="anh">ẢNh</th>
            ...
            <tbody>
                <?php tableProductCategories($category_product) ?>
            </tbody>
        </table>
                </div>
...
@section('script-bottom')
    <script type="text/javascript">
        $('#searchCateProa').on('keyup',function(){
            $value = $(this).val();
            // alert($value);
            $.ajax({
                type: 'get',
                url: '{{ route('search.cate.product') }}',
                data: {
                    'search': $value,
                },
       ...
       $('#actions').on('change',function() {
        if ($("#actions option:selected").val() == 1) {
            ...
            if (idsArr.length <=0)  {  
                ...
                if(confirm("Bạn có chắc muốn xoá danh mục sản phẩm đã chọn")){  
                    var strIds = idsArr.join(","); 
                    $.ajax({
                        url: '{{route('delete-multiple-category-product.product')}}',
                        type: 'GET',
