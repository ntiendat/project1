...
class LoginController extends Controller
{
    ...
    public function postLogin(Request $request)
    {
        ...
        //     $rememberTokenCookieKey = Auth::guard('pd-cmsweb')->getRecallerName();
		...
		if (Auth::attempt($arr)){
	    	...
	    	return redirect()->back()->with('danger','Email hoặc mật Khâu không đúng')->withInput();;
	    }
    ...
    public function dashboard(Request $request)
    {
        ...
        //                     ->backgroundcolor(['#e0a24c']);
        $post_counts = Post::orderBy('count','desc')->paginate(10);
        if ($request->ajax()) {
            return view('Admin.Dashboard.presult', compact('post_counts'));
        }
        $product_counts = Product::orderBy('count','desc')->paginate(10);
        // dd($product_counts);
        // dd($product_counts);
        // if ($request->ajax()) {
        //     return view('Admin.Dashboard.presult_product', compact('product_counts'));
        // }
    ...
    public function historyView($id){
        ...
        $data['history_posts_name'] = History::join('post','post.id','=','history.link_id')
                                            ->where('link_id',$id)->where('history.type',1)
                                            ->orderBy('history.id','desc')
                                            ->select('post.title')
                                            ->first();
    ...
    public function historyViewProduct($id){
        $data['history_products'] = History::join('product','product.id','=','history.link_id')
                                            ...
                                            ->select('history.*')
                                            ->paginate(10);
        $data['history_products_name'] = History::join('product','product.id','=','history.link_id')
                                            ->where('link_id',$id)->where('history.type',2)
                                            ->orderBy('history.id','desc')
